...
import notteshock.deviceinfo.components.ActionBarPopupWindow;
...
public class ActionBarMenuItem extends FrameLayout {
    ...
    private int menuHeight = InfoUtilities.dp(16);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getActionMasked() == MotionEvent.ACTION_DOWN) {
            if (hasSubMenu() && (popupWindow == null || popupWindow != null && !popupWindow.isShowing())) {
                ...
                InfoUtilities.runOnUIThread(showMenuRunnable, 200);
            }
    ...
    public TextView addSubItem(int id, String text, int icon) {
        ...
        if (!InfoUtilities.isRTL()) {
            textView.setGravity(Gravity.CENTER_VERTICAL);
        } else {
            textView.setGravity(Gravity.CENTER_VERTICAL | Gravity.RIGHT);
        }
        textView.setPadding(InfoUtilities.dp(16), 0, InfoUtilities.dp(16), 0);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 18);
        textView.setMinWidth(InfoUtilities.dp(196));
        textView.setTag(id);
        textView.setText(text);
        if (icon != 0) {
            textView.setCompoundDrawablePadding(InfoUtilities.dp(12));
            if (!InfoUtilities.isRTL()) {
        ...
        layoutParams.width = LayoutHelper.MATCH_PARENT;
        layoutParams.height = InfoUtilities.dp(48);
    ...
    public void toggleSubMenu() {
        ...
        if (showMenuRunnable != null) {
            InfoUtilities.cancelRunOnUIThread(showMenuRunnable);
        ...
        if (popupWindow == null) {
            ...
            popupLayout.measure(MeasureSpec.makeMeasureSpec(InfoUtilities.dp(1000), MeasureSpec.AT_MOST), MeasureSpec.makeMeasureSpec(InfoUtilities.dp(1000), MeasureSpec.AT_MOST));
    ...
    public boolean toggleSearch(boolean openKeyboard) {
        ...
        if (searchContainer.getVisibility() == VISIBLE) {
            if (listener == null || listener != null && listener.canCollapseSearch()) {
                ...
                InfoUtilities.hideKeyboard(searchField);
                if (listener != null) {
            ...
            if (openKeyboard) {
                InfoUtilities.showKeyboard(searchField);
            }
            if (listener != null) {
    ...
    public ActionBarMenuItem setIsSearchField(boolean value) {
        ...
        if (value && searchContainer == null) {
            ...
            layoutParams.height = LayoutHelper.MATCH_PARENT;
            layoutParams.leftMargin = InfoUtilities.dp(6);
            ...
            layoutParams2.height = InfoUtilities.dp(36);
            layoutParams2.rightMargin = InfoUtilities.dp(48);
            ...
            layoutParams2.width = InfoUtilities.dp(48);
    ...
    private void updateOrShowPopup(boolean show, boolean update) {
        ...
        if (showFromBottom) {
            getLocationOnScreen(location);
            int diff = location[1] - InfoUtilities.statusBarHeight + getMeasuredHeight() - menuHeight;
            offsetY = -menuHeight;
            if (diff < 0) {
                offsetY -= diff;
            }
        ...
        if (subMenuOpenSide == 0) {
            ...
            if (show) {
                popupWindow.showAsDropDown(this, -InfoUtilities.dp(10), offsetY);
            }
            if (update) {
                popupWindow.update(this, -InfoUtilities.dp(10), offsetY, -1, -1);
            }
        }
    }
}